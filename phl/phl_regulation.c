 ...
 *
 */
bool rtw_phl_regulation_set_6g_category(void *phl, enum rtw_power_limit_6g_info ri)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_regulation *rg = NULL;
	void *d = NULL;

	PHL_INFO("[REGU], set 6g category code = 0x%x\n", ri);
...
bool rtw_phl_query_regulation_info(void *phl, struct rtw_regulation_info *info)
{
	...
	info->category_6g = rg->category_6g;
...
u8 rtw_phl_get_cat6g_by_country(char *cntry)
{
	...
	idx = _country_index(cntry);
	if (MAX_COUNTRY_NUM == idx)
		return 0;

	return (u8)cdmap[idx].category_6g;
}
