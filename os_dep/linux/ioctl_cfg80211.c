...
#define RTW_CFG80211_DEV_PARAM_TYPE	struct wireless_dev *
#define RTW_CFG80211_DEV_PARAM_NAME	wdev
#define RTW_CFG80211_GET_ADAPTER(wdev)	((_adapter *)rtw_netdev_priv((wdev)->netdev))
...
static int cfg80211_rtw_add_key(struct wiphy *wiphy, RTW_CFG80211_DEV_PARAM_TYPE RTW_CFG80211_DEV_PARAM_NAME
	...
	, const u8 *mac_addr, struct key_params *params)
{
	...
	int ret = 0;
	_adapter *padapter = RTW_CFG80211_GET_ADAPTER(RTW_CFG80211_DEV_PARAM_NAME);
	struct net_device *ndev = RTW_CFG80211_GET_NETDEV(RTW_CFG80211_DEV_PARAM_NAME);
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
#ifdef CONFIG_TDLS
	...
	if (MLME_IS_STA(padapter)) {
		...
		/* RTW_INFO("@@@@@@@@@@ fw_state=0x%x, iftype=%d\n", pmlmepriv->fw_state, padapter->rtw_wdev->iftype); */
		ret = rtw_cfg80211_set_encryption(ndev, param);
	} else
...
static int cfg80211_rtw_get_key(struct wiphy *wiphy, RTW_CFG80211_DEV_PARAM_TYPE RTW_CFG80211_DEV_PARAM_NAME
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
	, int link_id
#endif
	, u8 keyid
...
#define GET_KEY_PARAM_ARG_E , mac_addr
	...
	_adapter *adapter = RTW_CFG80211_GET_ADAPTER(RTW_CFG80211_DEV_PARAM_NAME);
	struct net_device *ndev = RTW_CFG80211_GET_NETDEV(RTW_CFG80211_DEV_PARAM_NAME);
...
static int cfg80211_rtw_del_key(struct wiphy *wiphy, RTW_CFG80211_DEV_PARAM_TYPE RTW_CFG80211_DEV_PARAM_NAME
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
	, int link_id
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) || defined(COMPAT_KERNEL_RELEASE)
...
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37)) */
{
	...
	RTW_INFO(FUNC_NDEV_FMT" key_index=%d, addr=%pM\n",
		FUNC_NDEV_ARG(RTW_CFG80211_GET_NETDEV(RTW_CFG80211_DEV_PARAM_NAME)), key_index, mac_addr);
...
#define SET_DEF_KEY_PARAM_ARG , key_index
	...
	RTW_INFO(FUNC_NDEV_FMT
		SET_DEF_KEY_PARAM_FMT
		"\n", FUNC_NDEV_ARG(RTW_CFG80211_GET_NETDEV(RTW_CFG80211_DEV_PARAM_NAME))
		SET_DEF_KEY_PARAM_ARG
	);
...
void rtw_cfg80211_indicate_sta_assoc(_adapter *padapter, u8 *pmgmt_frame, uint frame_len)
{
...
#if defined(RTW_USE_CFG80211_STA_EVENT) || defined(COMPAT_KERNEL_RELEASE)
	{
		...
		#else
		cfg80211_new_sta(padapter->pnetdev, get_addr2_ptr(pmgmt_frame), &sinfo, GFP_ATOMIC);
		#endif
	}
...
void rtw_cfg80211_indicate_sta_disassoc(_adapter *padapter, const u8 *da, unsigned short reason)
{
	...
	#else
	cfg80211_del_sta(padapter->pnetdev, da, GFP_ATOMIC);
	#endif
#else /* defined(RTW_USE_CFG80211_STA_EVENT) */
...
static int	cfg80211_rtw_add_station(struct wiphy *wiphy, RTW_CFG80211_DEV_PARAM_TYPE RTW_CFG80211_DEV_PARAM_NAME,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 16, 0))
	u8 *mac,
#else
	const u8 *mac,
...
static int	cfg80211_rtw_del_station(struct wiphy *wiphy, RTW_CFG80211_DEV_PARAM_TYPE RTW_CFG80211_DEV_PARAM_NAME,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 16, 0))
	u8 *mac
...
static int	cfg80211_rtw_dump_station(struct wiphy *wiphy, RTW_CFG80211_DEV_PARAM_TYPE RTW_CFG80211_DEV_PARAM_NAME,
...
#endif
{
...
#else
	roc_cookie = ATOMIC_INC_RETURN(&pcfg80211_rochinfo->ro_ch_cookie_gen);
	*cookie = roc_cookie; /* cfg80211 reads the cookie back through the pointer */
	...
	rtw_phl_remain_on_ch_cmd(padapter, roc_cookie, wdev,
